...
class HtmlFixer
{
    ...
    /**
     * inline styles array of allowed css (if empty means ALL allowed)
     * @var array
     */
    ...
    private function fixQuotes($s)
    {
        ...
        for ($i = 0; $i < count($o[1]); $i++) {
            ...
            if ($t != "") {
                ...
                if (($t[strlen($t) - 1] != "\"") && ($t[strlen($t) - 1] != "'")) $out = str_replace($t . $lc,
                    $t . $q . $lc, $out);
            }
        }
        return $out;
        ...
        for ($i = 1; $i < count($this->matrix); $i++) {
            ...
            if ($this->matrix[$i]["tagType"] == "table") {
                ...
                if (in_array($parentType,
                    array("p", "b", "i", "font", "u", "small", "strong", "em", "tr", "table"))) $flag = true;
            }
            if ($flag) {
                $errorsCounter++;
