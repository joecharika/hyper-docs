 ...
 * @usage:
 * eg.1. @Validator rule1|rule2
 * eg.2. @Validator required|length(5)
 */
abstract class Validator
{
    ...
    public static function validate(object $object): object
    {
        ...
        foreach ((array)$object as $property => $value) {
            foreach (explode('|', Annotation::getPropertyAnnotation($class, $property, 'Validator')) as $rule) {
                ...
                elseif (Str::contains($rule, 'url'))
                    self::url($value, $errors, $property);
    ...
    protected static function email($value, array &$errors, $property): void
    {
        if (!filter_var($value, FILTER_VALIDATE_EMAIL))
            $errors[$property][] = 'Not a valid email';
    }

    protected static function url($value, array &$errors, $property): void
    {
        preg_match(
