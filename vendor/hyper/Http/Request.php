...
class Request
{
        ...
        foreach ($properties as $property => $value) {
            if (property_exists(Request::data(), $property)) {
                if (Annotation::getPropertyAnnotation($class, $property, 'File'))
                    $object->$property = !empty(@Request::files()->$property['tpm_name'])
                        ? Request::files()->$property
                        : Request::data()->$property;
                elseif (Annotation::getPropertyAnnotation($class, $property, 'Html'))
                    $object->$property = (new HtmlFixer)->getFixedHtml(Request::data()->$property);
                else
                    $object->$property = Request::data()->$property;
            }
        }
    ...
    /**
     * Get posted files as object
     * @return object
     */
    public static function files($array = false)
    {
        return $array ? $_FILES : (object)$_FILES;
    }
